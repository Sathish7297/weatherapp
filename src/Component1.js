...
const Weatherdetails = ({icon,temp,location,country,wind,lattitude,longitude,humidity,humidityIcon,windIcon}) => {
  ...
  // const[citynotfound,setcitynotfound]=useState(city)
...
const App = () => {
  ...
  const [wind,setwinds]=useState(0)
  ...
  settext(e.target.value)
    ...
    return (
      ...
      <div className=' input-container'>
        ...
        placeholder='search city' onChange={handlecheck} value={text} onKeyDown={handlekeydown}/>
        <div className='search-icon'>
          <img src={searchIcon} alt='searchicon' height={"20px"} onClick={()=>{search()}} loading={false} citynotfound={false}/>
        </div>
  
      </div>
      {citynotfound && <div className='ct'>citynotfound</div>}
